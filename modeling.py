...
class BertModel(object):
    ...
    def get_encoder_layers(self, sess, batch_features):
        all_unique_ids = []
        all_input_ids = []
        all_input_mask = []
        all_input_type_ids = []
        ...
        all_layers = sess.run(
            ...
            feed_dict={
                ...
                self.token_type_ids: all_input_type_ids
            }
        )

        # all_layers: batch_size * num_layers * max_seq_len * hidden_size
        all_layers = np.swapaxes(all_layers, 0, 1)
        assert len(all_layers) == len(all_unique_ids)

        predictions = []

        for unique_id, layer_wise_outputs in zip(all_unique_ids, all_layers):
            predictions.append(
                {
                    "unique_id": unique_id,
                    "layer-wise_outputs": layer_wise_outputs
                }
            )
    ...
    def embedding_postprocessor(
        ...
        if use_token_type:
            ...
            token_type_table = tf.get_variable(
                ...
                shape=[self.type_vocab_size, width],
                initializer=create_initializer(self.initializer_range))
            # This vocab will be small so we always do one-hot here, since it is always
            # faster for a small vocabulary.
            flat_token_type_ids = tf.reshape(token_type_ids, [-1])
            one_hot_ids = tf.one_hot(flat_token_type_ids, depth=self.type_vocab_size)
        ...
        output = layer_norm_and_dropout(output, self.hidden_dropout_prob)
        return output
